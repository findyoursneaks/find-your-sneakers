...
export default async function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section id="deals" className="dark"><div className="section"><div className="title inverse"><div><span>PRICE BATTLE</span><h2>Winning Prices Right Now</h2></div><Link className="link" href="/search">Enter the arena →</Link></div><p className="demo-note">Prices in the current catalogue are clearly marked demo data until authorised retailer feeds are approved and connected.</p><div className="dealgrid">{deals.map(({product:p,price,old})=>{const image=p.image_url||p.images?.[0];return <article className="deal" key={p.id}><div className="dealvisual"><b>WIN €{(old-(price||0)).toFixed(0)}</b>{image?<img src={image} alt={`${p.brands?.name||''} ${p.name}`}/>:<span>👟</span>}</div><small>{p.brands?.name}</small><h3>{p.name}</h3><div><strong>€{price?.toFixed(2)}</strong> <del>€{old.toFixed(2)}</del></div><Link className="deal-button" href={`/product/${p.slug}`}>View price battle</Link></article>})}</div></div></section>
...
function Header(){return <header className="nav"><Link className="logo war-logo" href="/"><span>SOLE</span><b>WAR</b></Link><nav><Link href="/search">Sneakers</Link><a href="#deals">Price Battles</a><a href="#brands">Brands</a><Link href="/search?gender=men">Men</Link><Link href="/search?gender=women">Women</Link><a href="#new">New Drops</a></nav><div className="actions"><Link href="/search" aria-label="Search">⌕</Link></div></header>}

function ProductGrid({items}:{items:Product[]}){return <div className="grid">{items.map(p=>{const price=bestPrice(p);const image=p.image_url||p.images?.[0];return <article className="card" key={p.id}><Link href={`/product/${p.slug}`}><div className="visual"><span className="badge">{p.is_new_release?'New challenger':p.is_trending?'Hot battle':'Price battle'}</span><div className="shoe">{image?<img src={image} alt={`${p.brands?.name||''} ${p.name}`}/>:<span>👟</span>}</div></div><small>{p.brands?.name}</small><h3>{p.name}</h3><p>{price ? <>Winning price <strong>€{price.toFixed(2)}</strong></> : 'Battle coming soon'}</p><span>{p.offers?.length || 0} stores competing</span><span className="compare">View battle <b>→</b></span></Link></article>})}</div>}

function Footer(){return <footer><div className="footbrand"><b className="war-logo"><span>SOLE</span><b>WAR</b></b><p>Every pair. Every price.<br/>One winner.</p></div><div className="footlinks"><Link href="/about">About</Link><a href="#how">How it works</a><a href="#brands">Brands</a><Link href="/legal/affiliate-disclosure">Affiliate Disclosure</Link><Link href="/legal/privacy">Privacy Policy</Link><Link href="/legal/terms">Terms</Link></div><div className="copyright">© 2026 SOLEWAR. Compare before you buy. We may earn a commission from eligible retailer links.</div></footer>}
